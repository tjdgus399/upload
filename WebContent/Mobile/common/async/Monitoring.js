...
function colorChange(param1) {
   this.param1 = param1;
   if (this.param1 == "R") {
      return "rgb(254,98,98)";
   } else if (this.param1 == "Y") {
      return "rgb(250,250,88)";
   } else {
      return "white";
   }
}
...
function InButtonCheck(IsCheckFlag, param1, param2, param3) {
   ...
   if (this.IsCheckFlag == "G") {
      ...
      var InsertButton = "<button id='flow' onclick=\"wtCautionPopUP("
            + "'"
            + param1
            + "',"
            + "'"
   ...
   if (request.readyState == 4 && request.status == 200) {
      ...
      if (createDivNumber != 0) {
         ...
         for (var i = 0; i < createDivNumber; i++) {
            creDiv.insertAdjacentHTML("beforeend", "<div id=\'monitorContent'\ class=\'ui-block-e'\> ");
         }
      ...
      for (var i = 1; i < result.length; i++) {
         ...
         for (var j = 0; j < 11; j++) {
            ...
            if (j == 1) {
               ...
               creDiv.children[i-1].style.backgroundPositon = "5% 5%";
...
function searchProcessUser(FarmID) {
   if (request.readyState == 4 && request.status == 200) {
      ...
      if (createDivNumber != 0) {
         ...
         // 검색된 양식장의 수조 수 만큼 div 영역을 생성함
         for (var i = 0; i < createDivNumber; i++) {
            creDiv.insertAdjacentHTML("beforeend", "<div class=\'ui-block-e'\ data-theme=\'b'\>");
         }
